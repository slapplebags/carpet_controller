...
const int LbuttonPin = 1; // The pin the button is connected to
const int RbuttonPin = 2; // The pin the button is connected to
const int CbuttonPin = 3; // The pin the button is connected to
...
const int heaterPinCW = 21; // Heater control pin
const int heaterPinCCW = 17; // Heater control pin
const int fanPin = 18; // Heater control pin
const int coilPin = 44; // Heater control pinsdfdf
...
//    Serial.println("engine on");
  ...
  // Update previousOutput with the current Output value for the next iteration
...
void loopAlternate1() {
    ...
    tft.fillScreen(TFT_BLACK);
    tft.setTextColor(TFT_GREEN, TFT_BLACK);
    tft.drawString("HeX Set Temp", 0, 80, 4);
    tft.drawString(String(settings.heSetpoint, 1), 0, 100, 4); // Show current heSetpoint settings);
}
...
void loopAlternate2() {
    ...
    tft.fillScreen(TFT_BLACK);
    tft.setTextColor(TFT_GREEN, TFT_BLACK);
    tft.drawString("Motor Runtime", 0, 80, 4);
    tft.drawString(String(settings.heMotorTime, 1), 0, 100, 4); // Show current heSetpoint settings);
}
...
void loopAlternate3() {
    ...
    tft.fillScreen(TFT_BLACK);
    tft.setTextColor(TFT_GREEN, TFT_BLACK);
    tft.drawString("Fan Set Temp", 0, 80, 4);
    tft.drawString(String(settings.fanSetpoint, 1), 0, 100, 4); // Show current heSetpoint settings);
}
...
void loopAlternate4() {
    ...
    tft.fillScreen(TFT_BLACK);
    tft.setTextColor(TFT_GREEN, TFT_BLACK);
    tft.drawString("E-Stop Set Temp", 0, 80, 4);
    tft.drawString(String(settings.coilSetpoint, 1), 0, 100, 4); // Show current heSetpoint settings);
}
